...
from indico_exporter.utils.datetime_utils import timestamp_to_iso
...
def serialize_workflow_to_csv(workflow_body, workflow_id: int):
    ...
    all_rows.sort(
        key=lambda r: (
            r["field_name"] == "" or r["field_name"] is None,
            r["field_name"] or "",
    ...
    write_csv(collapsed_rows, csv_filename, fieldnames, CSV_DELIMITER)
...
def _extract_rows_from_model(model, component_name, fields_mapping):
    ...
    agent_version = timestamp_to_iso(updated_at)
